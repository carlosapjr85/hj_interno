...
class ExpedicaoSupabaseUser extends BaseAuthUser {
  ExpedicaoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> expedicaoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ExpedicaoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
